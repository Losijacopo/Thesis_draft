...
Therefore, the principal comparison could be done between the derivative-based approach, which can be described as a lightweight method, and the standard SGBM method built in in OpenCV.
...
As a matter of fact, it allows to recover from calibration errors and noise, which are almost inevitable when images are taken in a real environment under normal conditions.  
...
Moreover, that feature supports the possibility of getting accurate results even in texture-less area of the scene, which are extremely challenging for pure stereo-matching based methods.
As a matter of fact, in the literature studied, that was a problem that many researchers tried to overcome in multiple ways, such as designing deep-learning based algorithm in the current years.
...
Thus, once it has been embedded in a stereo device with the same general features as the one used by the company, it can provide a dense and accurate 3D point cloud of the scanned scene with a potential real-time performance.\\
